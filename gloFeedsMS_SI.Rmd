...
allComps<-livFeeds%>%
  ...
  bind_rows(.,aqIn2018%>%
              mutate(product="aquaculture",
                     material=case_when(input == "fishmeal" ~ "Fishmeal",
...
Protein content of inputs to livestock and aquaculture feeds. Bolded ingredients (those with protein content greater than 20%) are replaced in growth projection scenarios, non-bolded are kept at constant inclusion rates to 2050.
...
pcTab<-pcIngr%>%
  mutate(material=case_when(ingredient=="soybeanmeal" ~ "Soybean meal",
                         ingredient=="rapeseedmeal" ~ "Rapeseed meal",
                         ingredient=="wheatbran" ~ "Wheat bran",
                         TRUE ~ ingredient))%>%
  select(material,proteinContent=`PC middle`)%>%
  left_join(allComps%>%
              filter(!duplicated(material))%>%
              select(material),.)%>%
  arrange(desc(proteinContent))%>%
  select(input=material,proteinContent)
...
Calorific content of feeds scenarios. "Growth" scenario feeds replace  ingredients indicated in table S1 only for growth in feeds above 2018 levels. Values are in kcal/kg of feed. 

```{r}

cals<-read_csv("data/calorificContent.csv")

cals2<-cals%>%
  filter(!input=="cottonseed meal")%>%
  group_by(input)%>%
  summarise(calories=mean(`calories (kcal/kg)`))

bauCals<-allComps%>%
  mutate(input=tolower(material))%>%
  left_join(.,cals2%>%
              mutate(input=tolower(input)))%>%
  mutate(calorificContent=incl*calories)
  
replsCals<-bauCals%>%
  select(-material)%>%
  filter(input %in% c("fishmeal","soybean meal","oilseed meals","rapeseed meal"))%>%
  left_join(.,pcTab%>%
              mutate(input=tolower(input)))%>%
  mutate(protIncl=incl*proteinContent/100)%>%
  group_by(product)%>%
  summarise(replProt=sum(protIncl))%>%
  mutate(replSoyCal=(replProt/0.46)*cals2[cals2$input=="soybean meal","calories"][[1]],
         replBactCal=(replProt/0.70)*cals2[cals2$input=="Bacteria","calories"][[1]],
         replYeastCal=(replProt/0.50)*cals2[cals2$input=="Yeast","calories"][[1]])%>%
  full_join(.,bauCals%>%
              filter(!input %in% c("fishmeal","soybean meal","oilseed meals","rapeseed meal"))%>%
              group_by(product)%>%
              summarise(totCal=sum(calorificContent)))%>%
  mutate(across(everything(), ~replace_na(.x, 0)))%>%
  mutate(soyGrowth=replSoyCal+totCal,
         bacteriaGrowth=replBactCal+totCal,
         yeastGrowth=replYeastCal+totCal)%>%
  select(product,soyGrowth,bacteriaGrowth,yeastGrowth)%>%
  left_join(.,bauCals%>%
              filter(!input %in% c("fishmeal","soybean meal","oilseed meals","rapeseed meal"))%>%
              group_by(product)%>%
              summarise(bauFeed=sum(calorificContent)))%>%
  select(product,bauFeed,soyGrowth,bacteriaGrowth,yeastGrowth)
...
```{r prodDat}
livProd<-read_csv("data/faoData/FAOSTAT_data_4-13-2021.csv") ## pulled data for the last 3 years: 2017,2018,2019 in case want to check out differences, start with 2019 tho --> replaced with all years of data in Aprilt
...
Projected greenhouse gas emissions of conventional verus alternative production methods for feed inputs. Alternative methods (dotted lines) refer to no till farming for crop inputs and yeast byproduct from biodiesel production from wheat. Real benefits of no-till farming vary by soil-type, location and over time, so these results are likely optimistic. 
...
Contribution of feeds to total production emissions for projected years
...
fdConCC<-prodEmiss%>%
  ...
  mutate(fdContr=totCCI/totGHG)%>%
  ggplot()+
  geom_line(aes(Year,fdContr*100,color=scenario),size=1)+
  scale_color_manual(values = fdScenCols)+
  scale_y_continuous(expand=c(0,0),limits=c(30,80))+
  # scale_x_continuous(breaks=c(2018,2020,2030,2040,2050),labels = c(2018,2020,2030,2040,2050))+
  labs(x="",y="Contribution of feeds\nto total GHG emissions (%)\n",title="Constant contribution")+
  theme_bw()+
...
fdConSC<-prodEmissD%>%
  ...
  scale_y_continuous(expand=c(0,0),limits=c(30,80))+
  # scale_x_continuous(breaks=c(2018,2020,2030,2040,2050),labels = c(2018,2020,2030,2040,2050))+
  labs(x="\n\nYear",y="",title="Shifting contribution")+
  theme_bw()+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"),
        legend.text = element_text(size=12),
...
fdConACS<-prodEmissD2%>%
  ...
  scale_y_continuous(expand=c(0,0),limits=c(30,80))+
  # scale_x_continuous(breaks=c(2018,2020,2030,2040,2050),labels = c(2018,2020,2030,2040,2050))+
  labs(x="",y="",title="Accelerated contribution shifts")+
  theme_bw()+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"),
        legend.text = element_text(size=12),
...
(fdConCC + fdConSC + fdConACS) + plot_layout(guides="collect")

```

***

### Estimating carbon taxes

These calculations are a rough estimate of how much of a carbon tax would be needed for yeast meals to be priced similar to soybean meals.

Data:

* Soybean meal prices: \$300 - \$550/tonne (World Bank)
* Current soybean meal price: \$470/tonne (World Bank)
  * Mean value = $440
* Yeast meal prices:  \$300 - \$800/tonne (alibaba.com)
  * Median value = `r (300+800)/2`
  
$$Price_{yeast}+ct(ghgEmissions_{yeast})=Price_{soy}+ct(ghgEmissions_{soy})$$

where $ct=carbon\ tax$, solving for $ct$, a carbon tax of only $\$21.36$ would be needed for yeast and soy to cost the same amount.